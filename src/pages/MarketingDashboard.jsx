...
export default function MarketingDashboard() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      if (result.status === 'success') {
        const marketingHistory = result.data.filter(log => 
          log.category?.toLowerCase() === 'marketing'
        );
        setHistoryLogs(marketingHistory);
      }
    } catch (error) {
      console.error("Gagal mengambil data log kampanye:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const stats = [
    { label: 'Total Campaigns', value: '38', color: 'text-slate-900 dark:text-white' },
    { label: 'Active Channels', value: '6', color: 'text-emerald-600 dark:text-emerald-500' },
    { label: 'Deployed Strategies', value: historyLogs.length.toString(), color: 'text-purple-600 dark:text-purple-400' },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter transition-colors">
            Marketing <span className="text-purple-600 dark:text-purple-500">Operations</span>
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="p-6 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-[2rem] hover:border-purple-500/30 transition-all group shadow-xl shadow-slate-200/50 dark:shadow-none">
      ...
      <div className="bg-white dark:bg-slate-900/20 border border-slate-200 dark:border-white/5 rounded-[2.5rem] overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-none transition-all">
        <div className="p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02] flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-slate-400 dark:text-slate-500 uppercase tracking-widest">Archive Mode</span>
          </div>
        </div>
        
        <div className="p-2 transition-all duration-300">
          {isLoading ? (
            <div className="p-10 text-center text-slate-400 text-sm animate-pulse font-mono tracking-widest">RETRIEVING CAMPAIGN ARCHIVE...</div>
            ...
            <div className="p-10 text-center">
              ...
              <h4 className="text-slate-900 dark:text-white font-bold text-sm uppercase italic">Archive Empty</h4>
              <p className="text-[10px] text-slate-500 mt-1 max-w-xs mx-auto uppercase tracking-tighter">
                No historical campaign data found.
              </p>
            </div>
          ) : (
